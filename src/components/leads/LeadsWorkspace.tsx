...
export default function LeadsWorkspace({
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-700"><Inbox className="h-4 w-4" /> Lead intake & response center</div><h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950">Leads</h1><p className="mt-1 text-sm text-slate-500">WATI replies, integrations, and flexible spreadsheet imports in one simple queue.</p></div><div className="flex flex-wrap gap-2">{isAdmin && <button onClick={() => setShowSources(true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold"><Settings2 className="h-4 w-4" /> Configure sources</button>}{isAdmin && <button onClick={() => setShowImport(true)} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white"><FileSpreadsheet className="h-4 w-4 text-amber-400" /> Import Excel / CSV</button>}</div></div>
