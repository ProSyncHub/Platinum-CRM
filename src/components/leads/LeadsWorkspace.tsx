...
export default function LeadsWorkspace({
  ...
  const basePath = mode === "wati" ? "/wati" : "/leads";
  const pageTitle = mode === "wati" ? "WATI Inbox" : "Leads";
  const pageKicker = mode === "wati" ? "WhatsApp response routing center" : "Lead intake & response center";
  const pageDescription =
    mode === "wati"
      ? "Meaningful WATI replies, auto-routed department queues, manager assignment, and optional chat sync."
      : "Manual imports, forms, integrations, and non-WATI lead sources in one simple queue.";
  const queueTitle = mode === "wati" ? "WATI routed queue" : "Lead queue";
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-700"><Inbox className="h-4 w-4" /> {pageKicker}</div><h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950">{pageTitle}</h1><p className="mt-1 text-sm text-slate-500">{pageDescription}</p></div><div className="flex flex-wrap gap-2">{isAdmin && <button onClick={() => setShowSources(true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold"><Settings2 className="h-4 w-4" /> {mode === "wati" ? "WATI setup" : "Configure sources"}</button>}{isAdmin && mode === "leads" && <button onClick={() => setShowImport(true)} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white"><FileSpreadsheet className="h-4 w-4 text-amber-400" /> Import Excel / CSV</button>}</div></div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label={mode === "wati" ? "All WATI items" : "All leads"} value={mode === "wati" ? stats.watiLeads : stats.totalLeads} icon={Users} /><MetricCard label="New queue" value={stats.newLeads} icon={Inbox} /><MetricCard label={mode === "wati" ? "Routed by WATI" : "Imported / API leads"} value={mode === "wati" ? stats.watiLeads : stats.totalLeads} icon={MessageCircle} /><MetricCard label="Questions waiting" value={stats.questionLeads} icon={CircleHelp} /></div>

      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs"><form onSubmit={(event) => { event.preventDefault(); setFilter("q", query.trim()); }} className={`grid gap-3 ${mode === "wati" ? "lg:grid-cols-[1fr_200px_auto]" : "lg:grid-cols-[1fr_220px_200px_auto]"}`}><label className="relative"><Search className="absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name, phone, email, or campaign…" className="w-full rounded-xl border border-slate-200 py-3 pl-10 pr-3" /></label>{mode === "leads" && <select value={filters.source} onChange={(e) => setFilter("source", e.target.value)} className="rounded-xl border border-slate-200 px-3 py-3"><option value="">All lead sources</option>{sources.map((source) => <option key={source.id} value={source.slug}>{source.name}</option>)}</select>}<select value={filters.status} onChange={(e) => setFilter("status", e.target.value)} className="rounded-xl border border-slate-200 px-3 py-3"><option value="">All statuses</option>{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status[0].toUpperCase() + status.slice(1)}</option>)}</select><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-5 py-3 font-bold text-slate-800"><Filter className="h-4 w-4" /> Apply</button></form></div>

      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xs"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="font-black text-slate-950">{queueTitle}</h2><p className="text-xs text-slate-500">Showing {leads.length} of {pagination.total} filtered items</p></div>{pending && <Loader2 className="h-4 w-4 animate-spin text-slate-500" />}</div><div className="overflow-x-auto"><table className="w-full min-w-[1220px] text-left"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Lead</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Response</th><th className="px-4 py-3">Campaign</th><th className="px-4 py-3">Received</th><th className="px-4 py-3">Assigned to</th><th className="px-4 py-3">CRM status</th><th className="px-4 py-3">Member</th></tr></thead><tbody>{leads.map((lead) => <tr key={lead.id} className="border-t border-slate-100 align-top hover:bg-slate-50/70"><td className="px-5 py-4"><p className="font-bold text-slate-950">{lead.fullName}</p><p className="mt-1 text-xs text-slate-500">{lead.phone || lead.email || "No contact information"}</p>{lead.phone && lead.email && <p className="text-xs text-slate-400">{lead.email}</p>}</td><td className="px-4 py-4"><span className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-bold text-indigo-700">{lead.source.name}</span>{lead.priority === "urgent" && <span className="mt-2 block w-fit rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-black uppercase text-rose-700">Urgent</span>}{lead.source.slug === "wati" && lead.phone && <button type="button" disabled={pending} onClick={() => startTransition(async () => { const result = await syncWatiConversationForLead(lead.id); if (!result.success) toast.error(result.error); else { toast.success(result.message); router.refresh(); } })} className="mt-2 block rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 text-[11px] font-bold text-emerald-800 disabled:opacity-50">Sync chat</button>}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${lead.responseCode === "has_question" ? "bg-rose-50 text-rose-700" : lead.responseCode === "already_paid" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{responseLabel(lead.responseCode, lead.responseText)}</span>{lead.notes && <p className="mt-2 max-w-xs whitespace-pre-line text-xs leading-5 text-slate-500">{lead.notes}</p>}</td><td className="max-w-48 px-4 py-4 text-sm text-slate-600">{lead.campaign || "—"}</td><td className="whitespace-nowrap px-4 py-4 text-xs text-slate-500">{dateLabel(lead.receivedAt)}</td><td className="px-4 py-4">{canAssignLeads ? <select value={lead.assignedToUser || ""} disabled={pending} onChange={(e) => { const next = e.target.value; if (!next) return; startTransition(async () => { const result = await assignLeadToUser(lead.id, next); if (!result.success) toast.error(result.error); else { toast.success(result.message || "Lead assigned."); router.refresh(); } }); }} className="max-w-56 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold"><option value="">{lead.assignedToName ? `${lead.assignedToName} · ${lead.assignedToDepartment || "team"}` : lead.assignedToDepartment ? `Queue: ${lead.assignedToDepartment}` : "Assign lead"}</option>{staff.map((person) => <option key={person.id} value={person.id}>{person.name} · {person.department}</option>)}</select> : <div><p className="text-xs font-bold text-slate-700">{lead.assignedToName || (lead.assignedToDepartment ? `${lead.assignedToDepartment} queue` : "Unassigned")}</p>{lead.assignedToDepartment && <p className="text-[11px] text-slate-400">{lead.assignedToDepartment}</p>}</div>}</td><td className="px-4 py-4"><select value={lead.status} disabled={pending} onChange={(e) => { const next = e.target.value; startTransition(async () => { const result = await updateLeadStatus(lead.id, next); if (!result.success) toast.error(result.error); else { toast.success("Lead status updated."); router.refresh(); } }); }} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold">{STATUS_OPTIONS.map((status) => <option key={status}>{status}</option>)}</select></td><td className="px-4 py-4">{lead.member ? <Link href={`/members/${lead.member.id}`} className="inline-flex items-center gap-1 text-xs font-bold text-indigo-700 hover:underline"><CheckCircle2 className="h-3.5 w-3.5" /> {lead.member.memberCode || lead.member.fullName}</Link> : <span className="text-xs text-slate-400">Not matched</span>}</td></tr>)}{!leads.length && <tr><td colSpan={8} className="px-6 py-16 text-center"><Inbox className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-bold text-slate-700">No items match these filters</p><p className="text-sm text-slate-500">Import a file or wait for an integration response.</p></td></tr>}</tbody></table></div><div className="flex items-center justify-between border-t border-slate-200 px-5 py-4"><span className="text-xs text-slate-500">Page {pagination.page} of {totalPages}</span><div className="flex gap-2"><Link aria-disabled={pagination.page <= 1} href={pagination.page > 1 ? pageHref(pagination.page - 1) : "#"} className={`inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-bold ${pagination.page <= 1 ? "pointer-events-none opacity-40" : "hover:bg-slate-50"}`}><ArrowLeft className="h-3.5 w-3.5" /> Previous</Link><Link aria-disabled={pagination.page >= totalPages} href={pagination.page < totalPages ? pageHref(pagination.page + 1) : "#"} className={`inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-bold ${pagination.page >= totalPages ? "pointer-events-none opacity-40" : "hover:bg-slate-50"}`}>Next <ArrowRight className="h-3.5 w-3.5" /></Link></div></div></div>
